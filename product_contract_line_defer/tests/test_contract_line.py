...
class Test(BaseCommon, TestContractBase):
    ...
    def test_contract_line_defer_onchange_product(self):
        """A deferred contract product defers the contract line on product change."""
        line = self.acct_line
        line.is_deferred = False
        line.product_id = self.product_1
        line._onchange_product_id_recurring_info()
        self.assertTrue(line.is_deferred)

    def test_sale_order_line_defer_default(self):
        """The company setting defers new sale order lines by default."""
        self.env.company.defer_contract_line_start = True
        defaults = self.env["sale.order.line"].default_get(["is_deferred"])
        self.assertTrue(defaults["is_deferred"])
