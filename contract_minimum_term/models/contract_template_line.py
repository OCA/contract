...
class ContractTemplateLine(models.Model):
    ...
    @api.constrains("min_term_interval", "is_auto_renew", "date_end")
    def _check_min_term_date_end(self):
        """A minimum term keeps the line open-ended.

        With an end date, ``is_auto_renew`` would be ambiguous: the standard
        renewal would extend the end date, while the minimum term renewal
        extends the commitment. A terminated line keeps its end date, but
        ``stop()`` switches ``is_auto_renew`` off.
        """
        for record in self:
            if (
                record.min_term_interval > 0
                and record.date_end
                and record.is_auto_renew
            ):
                raise ValidationError(
                    self.env._(
                        "The line %(line)s has a minimum term: it cannot have "
                        "an end date renewed automatically.",
                        line=record.name,
                    )
                )
