...
from .contract_line import EARLY_TERMINATION_GROUP
...
class ContractContract(models.Model):
    ...
    def _compute_min_term_dates(self):
        for rec in self:
            last_line = rec._get_min_term_lines().sorted("min_term_date_end")[-1:]
    ...
    def _get_min_term_lines(self):
        """The lines that actually have a minimum term.

        Lines whose minimum term is over keep their ``min_term_date_end``, so
        the termination notice still applies to them.
        """
        self.ensure_one()
        return self.contract_line_ids.filtered(
            lambda line: line.min_term_date_end and not line.is_canceled
        )
    ...
    def _terminate_contract(
        ...
        if before_min_term and not self.env.user.has_group(EARLY_TERMINATION_GROUP):
            raise UserError(
                self.env._(
        ...
        res = super(
            ContractContract,
            self.with_context(
                min_term_contract_termination=True,
                min_term_request_date=request_date,
            ),
        )._terminate_contract(
            terminate_reason_id,
            terminate_comment,
