...
class TestContractDiscounts(TestContractBase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.env.ref("product.group_discount_per_so_line").users = [(4, cls.env.user.id)]
    ...
    def test_invoice_lines_discount_fixed(self):
        ...
        self.assertEqual(invoice.invoice_line_ids.discount_fixed, 48)
        self.assertEqual(invoice.invoice_line_ids.discount, 0)
