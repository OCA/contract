...
class TestSaleOrder(TransactionCase):
    def setUp(self):
        super().setUp()
        self.test_owner = self.env["res.partner"].create({"name": "TestOwner"})
        self.test_location = self.env["fsm.location"].create(
            {"name": "Testlocation", "owner_id": self.test_owner.id}
        )
        self.test_customer = self.env["res.partner"].create(
            {"name": "TestCustomer", "service_location_id": self.test_location.id}
        )
