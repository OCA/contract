...
class ContractLine(models.Model):
    ...
    last_date_invoiced = fields.Date(
        string='Last Date Invoiced', copy=False
    )
    ...
    def _prepare_invoice_line(self, move_id=False, invoice_values=False):
        ...
        fpos = self.env['account.fiscal.position'].browse(invoice_values['fiscal_position_id'] or None)
        tax_ids = fpos.map_tax(
            self.product_id.taxes_id.filtered(lambda t: t.company_id.id == invoice_values['company_id'])
        )
        ...
        invoice_line_vals = {
            ...
            'account_id': accounts['income' if invoice_values['type'] == 'out_invoice' else 'expense'],
